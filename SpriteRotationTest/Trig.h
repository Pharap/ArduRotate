#pragma once

#include <FixedPoints.h>
#include <FixedPointsCommon.h>

using AngleResult = SQ7x8;

constexpr AngleResult AngleTable[64] PROGMEM
{
	0.0, 0.0245, 0.0491, 0.0736, 0.098, 0.1224, 0.1467, 0.171, 0.1951, 0.2191, 0.243, 0.2667, 0.2903, 0.3137, 0.3369, 0.3599, 0.3827, 0.4052, 0.4276, 0.4496, 0.4714, 0.4929, 0.5141, 0.535, 0.5556, 0.5758, 0.5957, 0.6152, 0.6344, 0.6532, 0.6716, 0.6895, 0.7071, 0.7242, 0.741, 0.7572, 0.773, 0.7883, 0.8032, 0.8176, 0.8315, 0.8449, 0.8577, 0.8701, 0.8819, 0.8932, 0.904, 0.9142, 0.9239, 0.933, 0.9415, 0.9495, 0.9569, 0.9638, 0.97, 0.9757, 0.9808, 0.9853, 0.9892, 0.9925, 0.9952, 0.9973, 0.9988, 0.9997
};

using Brads = uint8_t;

inline AngleResult lookupAngle(uint_fast8_t index)
{
    return AngleResult::fromInternal(pgm_read_word(&AngleTable[index & 0x3F]));
}

inline AngleResult fixedSin(Brads brads)
{
	const uint8_t quarter = ((brads >> 6) & 0x03);
	const uint8_t index = ((brads >> 0) & 0x3F);

	switch (quarter)
	{
		case 0: return lookupAngle(index);
		case 1: return lookupAngle(63 - index);
		case 2: return -lookupAngle(index);
		case 3: return -lookupAngle(63 - index);
		default: return 0;
	}
}

inline AngleResult fixedCos(Brads brads)
{
	const uint8_t quarter = ((brads >> 6) & 0x03);
	const uint8_t index = ((brads >> 0) & 0x3F);

	switch (quarter)
	{
		case 0: return lookupAngle(63 - index);
		case 1: return -lookupAngle(index);
		case 2: return -lookupAngle(63 - index);
		case 3: return lookupAngle(index);
		default: return 0;
	}
}